...
const checkInput = (user: UserObj, setIsError: Function, setErrorMessage: Function, setProcess: Function, objectKey: Array<ObjectKey>, formType: string,) => {
  let error = false;
  // check for empty input
...
const processSubmit = async (formType: string, endPoint: string | null, user: UserObj, resetPasswordToken: string | undefined, setIsError: Function, setErrorMessage: Function,) => {
  ...
  else if (formType === 'loginWithEmail') {
    response = await signIn('credentials', {
      ...user,
      type: 'email',
      redirect: false,
